 ...
 *            validation timeout in milliseconds (default 20000)
 */
public record DatabaseConfig(String url, String user, String password, int maxPoolSize, int minIdle,
		long connectionTimeoutMs, long idleTimeoutMs, long maxLifetimeMs, long validationTimeoutMs) {
	...
	public static DatabaseConfig from(final EtherConfig config) {
		...
		final var maxPoolSize = config.get("DB_MAX_POOL_SIZE").map(Integer::parseInt).orElse(6);
		final var minIdle = config.get("DB_MIN_IDLE").map(Integer::parseInt).orElse(2);
		final var connectionTimeoutMs = config.get("DB_CONNECTION_TIMEOUT_MS").map(Long::parseLong).orElse(30000L);
		final var idleTimeoutMs = config.get("DB_IDLE_TIMEOUT_MS").map(Long::parseLong).orElse(600000L);
		final var maxLifetimeMs = config.get("DB_MAX_LIFETIME_MS").map(Long::parseLong).orElse(1800000L);
		final var validationTimeoutMs = config.get("DB_VALIDATION_TIMEOUT_MS").map(Long::parseLong).orElse(20000L);

		return new DatabaseConfig(url, user, password, maxPoolSize, minIdle, connectionTimeoutMs, idleTimeoutMs,
				maxLifetimeMs, validationTimeoutMs);
	}
