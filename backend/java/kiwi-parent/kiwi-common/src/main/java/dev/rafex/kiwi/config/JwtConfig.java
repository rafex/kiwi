 ...
 *
 * @param issuer
 *            token issuer (default "dev.rafex.kiwi")
 * @param audience
 *            token audience (default "kiwi-backend")
 * @param secret
 *            JWT secret (required, auto-generated if not provided)
 * @param ttlSeconds
 *            token time-to-live in seconds (default 3600)
 * @param appTtlSeconds
 *            app token time-to-live in seconds (default 1800)
 */
public record JwtConfig(String issuer, String audience, String secret, long ttlSeconds, long appTtlSeconds) {
	...
	public static JwtConfig from(final EtherConfig config) {
		final var issuer = config.get("JWT_ISS").orElse("dev.rafex.kiwi");
		final var audience = config.get("JWT_AUD").orElse("kiwi-backend");
		final var secret = resolveJwtSecret(config.get("JWT_SECRET").orElse(null));
		final var ttlSeconds = config.get("JWT_TTL_SECONDS").map(Long::parseLong).orElse(3600L);
		final var appTtlSeconds = config.get("JWT_APP_TTL_SECONDS").map(Long::parseLong).orElse(1800L);

		return new JwtConfig(issuer, audience, secret, ttlSeconds, appTtlSeconds);
	}

	/**
	 * If JWT_SECRET is not configured or uses the known default value, generate a
	 * random 256-bit secret and emit a warning. The generated secret does not
	 * persist between restarts — all issued tokens become invalid. In production
