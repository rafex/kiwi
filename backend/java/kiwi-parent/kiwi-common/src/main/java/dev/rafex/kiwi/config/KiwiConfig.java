 ...
 * }</pre>
 * <p>
 * For testing, use {@link #loadFrom(EtherConfig)} with a
 * {@code MapConfigSource}:
 * 
 * <pre>{@code
 * Map<String, String> testValues = Map.of("DB_URL", "jdbc:h2:mem:test", "JWT_SECRET", "test-secret-32-chars");
 * EtherConfig testSource = EtherConfig.of(new MapConfigSource("test", testValues));
 ...
 * @see ServerConfig
 */
public record KiwiConfig(DatabaseConfig database, JwtConfig jwt, AuthConfig auth, LoggingConfig logging,
		ServerConfig server) {
	public KiwiConfig {
	...
	/**
	 * Load configuration from default sources (environment variables and system
	 * properties).
	 * <p>
	 * Returns a singleton instance. Subsequent calls return the same instance
	 * unless {@link #reset()} has been called.
	 * <p>
	 * Example:
	 * 
	 * <pre>{@code
	 ...
	 * <p>
	 * This method is useful for testing or when you need to load configuration from
	 * custom sources (e.g., a {@code MapConfigSource} with test values).
	 * <p>
	 * Example for testing:
	 * 
	 * <pre>{@code
	 ...
	 *
	 * EtherConfig testSource = EtherConfig.of(new MapConfigSource("test", testConfig));
	 * KiwiConfig config = KiwiConfig.loadFrom(testSource);
	 * }</pre>
	 *
	 * @param config
	 *            the EtherConfig source to load from
	 ...
	 * <p>
	 * This method is useful when you need to ensure configuration comes exclusively
	 * from environment variables (not system properties).
	 * <p>
	 * Example:
	 * 
	 * <pre>{@code
