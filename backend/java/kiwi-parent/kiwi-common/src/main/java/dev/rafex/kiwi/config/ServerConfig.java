...
public record ServerConfig(int port, int maxThreads, int minThreads, int idleTimeoutMs, String threadPoolName,
	...
	public static ServerConfig from(final EtherConfig config) {
		...
		final var port = config.get("PORT").map(Integer::parseInt).orElse(8080);
		final var maxThreads = config.get("HTTP_MAX_THREADS").map(Integer::parseInt).orElse(Math.max(cpus * 2, 16));
		final var minThreads = config.get("HTTP_MIN_THREADS").map(Integer::parseInt).orElse(4);
		final var idleTimeoutMs = config.get("HTTP_IDLE_TIMEOUT_MS").map(Integer::parseInt).orElse(30_000);
		final var threadPoolName = config.get("HTTP_POOL_NAME").orElse("kiwi-http");
		...
		return new ServerConfig(port, maxThreads, minThreads, idleTimeoutMs, threadPoolName, environment,
